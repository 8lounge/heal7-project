...
# Mock 데이터베이스 및 사용자 인증 (실제 구현 전까지 사용)
class MockDB:
    """Mock 데이터베이스 클래스"""
        ...
        class MockResult:
            def fetchall(self):
                return [{
                    ...
                    'keyword': '뱀',
                    'category_name': '동물',
                    'traditional_meaning': '뱀꿈은 전통적으로 길몽으로 여겨집니다. 재물운 상승, 지혜 획득, 치유의 의미가 있습니다.',
                    'modern_meaning': '변화와 변신의 상징입니다. 새로운 시작이나 성장을 의미하며, 때로는 숨겨진 적이나 위험을 나타내기도 합니다.',
                    'psychological_meaning': '무의식의 힘, 원시적 에너지, 억압된 욕구의 상징입니다.',
                    'fortune_aspect': 'positive',
                    'confidence_score': 0.85,
                    'related_keywords': ['용', '도마뱀', '거북이'],
                    'lucky_numbers': [3, 7, 21],
                    'search_frequency': 230
                }]
        return MockResult()
...
async def search_dreams(
    ...
    try:
        ...
        SELECT 
            ...
            di.search_frequency,
            0.0 as match_score
        FROM dream_service.dream_interpretations di
...
async def get_dream_categories(db: Session = Depends(get_db)):
    ...
    try:
        ...
        SELECT 
            ...
            description,
            parent_id,
            (SELECT COUNT(*) FROM dream_service.dream_interpretations WHERE category_id = dc.id) as dream_count
        FROM dream_service.dream_categories dc
...
async def get_dream_combinations(
    ...
    try:
        # 먼저 해당 꿈의 키워드 조회
        dream_query = "SELECT keyword FROM dream_service.dream_interpretations WHERE id = :dream_id"
        ...
        FROM dream_service.dream_combinations 
...
async def record_user_dream(
    ...
    try:
        ...
        SELECT id FROM dream_service.dream_interpretations 
...
async def get_trending_dreams(
    ...
    try:
        ...
        FROM dream_service.dream_search_stats
...
async def bulk_import_dreams(
    ...
    try:
        ...
        for dream_data in dreams_data:
            # 중복 체크
            existing_query = "SELECT id FROM dream_service.dream_interpretations WHERE keyword = :keyword"
...
async def search_multi_perspective_dreams(
    ...
    try:
        ...
        WITH dream_perspective_counts AS (
            ...
            FROM dream_service.dream_interpretations di
            INNER JOIN dream_service.dream_perspective_interpretations dpi ON di.id = dpi.dream_id
        ...
        SELECT * FROM dream_service.dream_perspective_counts
...
async def get_multi_perspective_dream(
    ...
    try:
        ...
        FROM dream_service.dream_perspective_interpretations dpi
        INNER JOIN interpretation_perspectives ip ON dpi.perspective_id = ip.id
        ...
        FROM dream_service.dream_comparison_analysis
...
async def generate_comparison_analysis(dream_id: str, perspectives: Dict[str, PerspectiveInterpretation], db: Session) -> ComparisonAnalysis:
    ...
    try:
        ...
        # 분석 결과를 데이터베이스에 저장
        save_query = """
        INSERT INTO dream_service.dream_comparison_analysis 
