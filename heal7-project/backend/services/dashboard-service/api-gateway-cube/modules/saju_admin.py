...
# from ..core.saju_admin_settings import ...
...
async def save_all_settings(
    ...
    try:
        # 데이터 디렉토리 생성
        os.makedirs(os.dirname(SETTINGS_FILE_PATH), exist_ok=True)
...
async def get_system_status(token: str = Depends(verify_admin_token)) -> Dict[str, Any]:
    ...
    return {
        ...
        "current_config": {
            "timezone_system": settings.get("timezone_system", "standard"),
            "logic_type": settings.get("logic_type", "hybrid"),
            "default_country": settings.get("default_country", "KR"),
            "use_kasi_precision": settings.get("use_kasi_precision", True),
            "manseeryeok_count": settings.get("manseeryeok_count", 60000),
...
async def backup_current_settings(token: str = Depends(verify_admin_token)) -> Dict[str, str]:
    ...
    try:
        ...
        # 백업 디렉토리 생성
        os.makedirs(os.dirname(backup_path), exist_ok=True)
