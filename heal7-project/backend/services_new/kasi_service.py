...
class KASIAPIResponse(BaseModel):
    ...
    def items(self) -> List[Dict[str, Any]]:
        """응답에서 아이템 목록 추출"""
        try:
            return self.response.get("body", {}).get("items", [])
    ...
    def total_count(self) -> int:
        """총 아이템 수"""
        try:
            return self.response.get("body", {}).get("totalCount", 0)
        except (KeyError, AttributeError):
            return 0
...
class KASIService:
    ...
    async def _call_kasi_api(
        ...
        url = f"{self.settings.kasi_base_url}/getRestDeInfo"
        params = {
            "serviceKey": self.settings.kasi_api_key,
            "solYear": year,
            "solMonth": month,
            "numOfRows": num_rows,
            "_type": "json"
        ...
        try:
            ...
            logger.error(f"KASI API HTTP 오류 ({e.response.status_code}): {e.response.text}")
            raise
        ...
        try:
            ...
            for item in api_response.items:
                try:
                    ...
                    for code, (name, term_type) in self.SOLAR_TERMS.items():
            ...
            if terms and use_cache:
                await self._cache_solar_terms(year, month, terms, self.settings.cache_ttl)
