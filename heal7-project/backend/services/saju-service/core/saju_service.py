...
class SajuResult(BaseModel):
    ...
    def is_strong_day_master(self) -> bool:
        ...
        supporting_count = self.get(day_element, 0)
        
        # 간단한 강약 판단 (실제로는 더 복잡한 로직 필요)
        total_elements = sum(self.values())
...
class SajuService:
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        # 의존 서비스들 상태 확인
        kasi_health = await self.health_check()
        # AI 서비스는 향후 통합 예정 - 현재 보류  
        # ai_health = await self.health_check()
        db_health = await self.health_check()
    ...
    def _get_month_pillar(self, year: int, month: int, solar_term: Optional[SolarTermData] = None) -> Tuple[str, str]:
        ...
        year_cheonan_index = self.index(year_cheonan)
    ...
    def _get_time_pillar(self, day_cheonan: str, hour: int) -> Tuple[str, str]:
        ...
        # 시간대별 지지 결정
        hour_jiji = None
        for jiji, (start, end) in self.items():
        ...
        # 시간 천간 계산
        day_index = self.index(day_cheonan)
        jiji_index = self.index(hour_jiji)
    ...
    def _calculate_sipsin(self, day_cheonan: str, target_cheonan: str) -> str:
        ...
        if day_cheonan not in self.SIPSIN_RELATIONS:
            ...
            day_index = self.index(day_cheonan)
            target_index = self.index(target_cheonan)
    ...
    def _calculate_daeun(self, birth_info: BirthInfo, day_pillar: Pillar) -> Dict[str, Any]:
        ...
        is_yang_year = self.index(year_cheonan) % 2 == 0
    ...
    async def calculate_saju(self, birth_info: BirthInfo) -> SajuResult:
        ...
        try:
            ...
            solar_term = None
            try:
                solar_term = await self.find_solar_term_by_date(birth_info.birth_datetime)
                if solar_term:
    ...
    async def analyze_with_ai(
        ...
        saju_data = {
            "birth_datetime": saju_result.birth_datetime.strftime("%Y년 %m월 %d일 %H시"),
            "palcha": saju_result.palcha,
            "day_pillar": str(saju_result.day_pillar),
            "day_master": saju_result.day_master,
            "day_master_element": saju_result.value,
        ...
        traditional_analysis = {
            ...
            "dominant_sipsin": saju_result.get("dominant"),
            "sinsal": saju_result.sinsal,
    ...
    async def save_saju_result(self, saju_result: SajuResult, user_id: Optional[str] = None) -> str:
        ...
        try:
            ...
            save_data = {
                ...
                "created_at": saju_result.isoformat()
            }
            
            # 실제 구현에서는 데이터베이스에 저장
            save_id = f"saju_{int(saju_result.timestamp())}"
    ...
    def __repr__(self) -> str:
        ...
        return f"SajuService(status={status}, version=1.0)"