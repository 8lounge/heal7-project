...
class FileUtils:
    ...
    def extract_text_from_pdf(pdf_file_path: str) -> str:
        ...
        try:
            if not PyPDF2:
                return f"[PDF 처리 라이브러리 없음] {os.basename(pdf_file_path)}"
            ...
            return f"[PDF 처리 오류: {str(e)}] {os.basename(pdf_file_path)}"
    ...
    def extract_text_from_docx(docx_file_path: str) -> str:
        ...
        try:
            if not Document:
                return f"[DOCX 처리 라이브러리 없음] {os.basename(docx_file_path)}"
    ...
    def extract_text_from_hwp(hwp_file_path: str) -> str:
        ...
        filename = os.basename(hwp_file_path)
    ...
    def extract_text_from_document(file_path: str) -> Dict[str, Any]:
        ...
        file_ext = FileUtils.get_file_extension(os.basename(file_path)).lower()
