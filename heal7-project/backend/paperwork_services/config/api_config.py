...
class APIConfig:
    ...
    def __init__(self):
        ...
        # 네이버 OCR API 설정
        self.naver_ocr = {
            'url': os.getenv('NAVER_OCR_URL', 'https://your-ocr-api.apigw.ntruss.com/custom/v1/your-model/general'),
            'secret_key': self.api_keys.get('NAVER_OCR_SECRET_KEY'),
            'invoke_url': self.api_keys.get('NAVER_OCR_INVOKE_URL'),
            'timeout': 60,
            'max_retries': 3
        }
    ...
    def is_ai_model_configured(self, model_id: str) -> bool:
        """특정 AI 모델이 설정되었는지 확인"""
        model_config = self.ai_models.get(model_id)
